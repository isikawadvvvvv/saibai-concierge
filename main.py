...
def calculate_gdd(weather_data, base_temp=10.0):
    ...
    gdd = 0
    if 'temperature_2m_max' in weather_data['daily'] and 'temperature_2m_min' in weather_data['daily']:
...
（例：ミニトマトを追加）""")
    ...
    （例：ミニトマトの状態）""")
        ...
        print(f"An error occurred in handle_message: {e}")
        # エラーが発生した場合でも、ユーザーに何かしらの応答を返すことが望ましい
        with ApiClient(line_config) as api_client:
            line_bot_api = MessagingApi(api_client)
            line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token, messages=[TextMessage(text="申し訳ありません、エラーが発生しました。もう一度お試しください。")]))
...
def handle_postback(event):
    ...
    try:
        ...
        print(f"An error occurred in handle_postback: {e}")
        with ApiClient(line_config) as api_client:
            line_bot_api = MessagingApi(api_client)
            line_bot_api.reply_message(ReplyMessageRequest(reply_token=event.reply_token, messages=[TextMessage(text="記録中にエラーが発生しました。")]))
